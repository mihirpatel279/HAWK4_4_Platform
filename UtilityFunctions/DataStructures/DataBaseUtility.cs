...
namespace Utilities
{
    public class DataBaseUtility
    {
        ...
        {
             String   _connectionString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source= Prototype.accdb";
             //String   _connectionString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source= PACS.accdb";
             //  if ( MainForm.radioButton_MS_SQL.Checked) 
            //  string _connectionString = @"Provider=SQLOLEDB;Data Source=mis-sql.uhcl.edu; Initial Catalog=PACS; User ID= pacs; Password=pacs2016!!"; 
        ...
        public static int GetLastNumber(string tableName, int _column, int lowerBound, int upperbound)
        {
          ...
          //  string oldTarget = "999";
            ...
            if (oldTarget != "")
                try
                {
                    string cmdStr = String.Format ("select * from {0} order by DocNum",  tableName);
                    List<string> _list = GetList(cmdStr );
                    int lastrow = _list.Count - 1;
                    string[] lastrowStr = _list[lastrow].Split(delim);
                    target = lastrowStr[0];   //_column];
