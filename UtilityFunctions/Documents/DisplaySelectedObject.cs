...
namespace Utilities.Documents
{
   ...
   //         LoadBBasicDocInfo();
       ...
       public virtual void LoadBBasicDocInfo()
        ...
        protected void button_ResetLabels_Click(object sender, EventArgs e)
        {
          List<string>  _dataNames = Utilities.DataBaseUtility.GetList(textBox_Cmd.Text);
            
                if (_dataNames.Count < 2)
                {
                    MessageBox.Show("  error in reading table : " + textBox_TableName.Text);
                    return;
                }
                string[] _labelValues = _dataNames[0].Split(',');
                label_ExtAgent.Text = _labelValues[2];
                label_IntAgent.Text = _labelValues[3];
                label4.Text = _labelValues[4];
                label5.Text = _labelValues[5];
                label6.Text = _labelValues[6];
                label7.Text = _labelValues[7];
                label8.Text = _labelValues[8];
                label9.Text = _labelValues[9];
        }
        ...
        protected void button_Display_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                textBox_NumValue1.Text = _documentValues[8].ToString();     //  expectedAmount 
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
         ...
         //   new Operations.Inspections. EditDetailObject().Show(); 
        }
        ...
        {
            new ChangeDocument(base.ident, textBox_Cmd.Text, textBox_TableName.Text, textBox_DocNum.Text, textBox_DocType.Text).Show();
        }
